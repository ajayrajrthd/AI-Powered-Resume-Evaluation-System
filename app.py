...
class ResumeApp:
    ...
    def add_footer(self):
        ...
        st.markdown(
            "<hr style='margin-top: 50px; margin-bottom: 20px;'>", unsafe_allow_html=True)
        ...
        st.markdown(f"""
            <div class="profile-section">
                <img src={image_base64}
                     alt="Ajayraj Rathod"
                     class="profile-image">
                <h2 class="profile-name">Ajayraj Rathod</h2>
                <p class="profile-title">Data Analyst & Data Science Enthusiast</p>
                <div class="social-links">
                    <a href="https://github.com/ajayrajrthd" class="social-link" target="_blank">
                        <i class="fab fa-github"></i>
                    </a>
                    <a href="https://www.linkedin.com/in/ajayraj-rathod-b72996248/" class="social-link" target="_blank">
                        <i class="fab fa-linkedin"></i>
                    </a>
                    <a href="mailto:ajayrajrthd7@gmail.com" class="social-link" target="_blank">
                        <i class="fas fa-envelope"></i>
                    </a>
                </div>
                <p class="bio-text">
                    A passionate person focused on building scalable and sustainable social and technical systems. A strong interest in Data Analysis, Data Science, Machine Learning, and Software Development to deliver impactful data-driven end-user solutions. Always looking to build skills and use analytics to solve real-world problems and help create meaningful user experiences.
                </p>
            </div>
        """, unsafe_allow_html=True)
                        ...
                        try:
                            if uploaded_file.type == "application/pdf":
                                try:
                                    ...
                                    st.info(
                                        "Trying alternative PDF extraction method...")
                                    # Try AI analyzer as backup
                                    try:
                                        text = self.ai_analyzer.extract_text_from_pdf(
                                            uploaded_file)
                                    except Exception as backup_error:
                                        st.error(
                                            f"All PDF extraction methods failed: {str(backup_error)}")
                                        return
                                ...
                                try:
                                    ...
                                    st.error(
                                        f"DOCX extraction failed: {str(docx_error)}")
                                    # Try AI analyzer as backup
                                    try:
                                        text = self.ai_analyzer.extract_text_from_docx(
                                            uploaded_file)
                                    except Exception as backup_error:
                                        st.error(
                                            f"All DOCX extraction methods failed: {str(backup_error)}")
                                        return
                            else:
                            ...
                            if not text or text.strip() == "":
                                st.error(
                                    "Could not extract any text from the uploaded file. Please try a different file.")
                                return
                        except Exception as e:
                ...
                if analyze_ai:
                    with st.spinner(f"Analyzing your resume with {ai_model}..."):
                        ...
                        try:
                                ...
                                if analysis_result and "error" not in analysis_result:
                                    ...
                                    with col1:
                                        ...
                                        fig1 = go.Figure(go.Indicator(
                                            ...
                                            gauge={
                                                ...
                                                'steps': [
                                                    {'range': [
                                                        0, 40], 'color': 'rgba(255, 68, 68, 0.2)'},
                                                    {'range': [
                                                        40, 60], 'color': 'rgba(255, 165, 0, 0.2)'},
                                                    {'range': [
                                                        60, 80], 'color': 'rgba(255, 214, 0, 0.2)'},
                                                    {'range': [
                                                        80, 100], 'color': 'rgba(76, 175, 80, 0.2)'}
                                                ],
                                                'threshold': {
                                        ...
                                        st.markdown(
                                            f"<div style='text-align: center; font-weight: bold;'>{status}</div>", unsafe_allow_html=True)
                                    ...
                                    with col2:
                                        ...
                                        fig2 = go.Figure(go.Indicator(
                                            ...
                                            domain={'x': [0, 1], 'y': [0, 1]},
                                            title={
                                                'text': "ATS Optimization Score", 'font': {'size': 16}},
                                            gauge={
                                                'axis': {'range': [0, 100], 'tickwidth': 1},
                                    ...
                                    if st.session_state.get('used_custom_job_desc', False) and custom_job_description:
                                        ...
                                        job_match_score = analysis_result.get(
                                            "job_match_score", 0)
                                        if not job_match_score and "job_match" in analysis_result:
                                            job_match_score = analysis_result["job_match"].get(
                                                "score", 0)
                                            ...
                                            with col1:
                                                ...
                                                fig3 = go.Figure(go.Indicator(
                                                    ...
                                                    domain={
                                                        'x': [0, 1], 'y': [0, 1]},
                                                    title={
                                                        'text': "Job Match Score", 'font': {'size': 16}},
                                                    gauge={
                                                        'axis': {'range': [0, 100], 'tickwidth': 1},
                                                ...
                                                st.markdown(
                                                    f"<div style='text-align: center; font-weight: bold;'>{match_status}</div>", unsafe_allow_html=True)
                                        ...
                                        if section in formatted_analysis:
                                            ...
                                            for next_sec in section_styles.keys():
                                                if next_sec != section and next_sec in formatted_analysis.split(style)[1]:
                                                    split_text = formatted_analysis.split(
                                                        style)[1].split(next_sec)
                                                    formatted_analysis = formatted_analysis.split(
                                                        style)[0] + style + split_text[0] + "</div></div>" + next_sec + "".join(split_text[1:])
                                                    next_section = True
                                                    break
                                            if not next_section:
                                    ...
                                    if formatted_analysis.endswith("</div>"):
                                        ...
                                        open_tags = formatted_analysis.count(
                                            "<div")
                                        close_tags = formatted_analysis.count(
                                            "</div>")
                                    ...
                                    formatted_analysis = formatted_analysis.replace(
                                        "&lt;div&gt;", "")
                                    formatted_analysis = formatted_analysis.replace(
                                        "<div>", "<div>")  # Ensure proper opening
                                    formatted_analysis = formatted_analysis.replace(
                                        "</div>", "</div>")  # Ensure proper closing
                            ...
                            st.error(
                                f"Error during AI analysis: {str(ai_error)}")
    ...
    def render_home(self):
        ...
        with col2:
            if st.button("Get Started", key="get_started_btn",
                         ...
                         type="primary",
                         use_container_width=True):
                cleaned_name = "RESUME ANALYZER".lower().replace(
                    " ", "_").replace("🔍", "").strip()
        ...
        with st.sidebar:
            ...
            for page_name in self.pages.keys():
                if st.button(page_name, use_container_width=True):
                    cleaned_name = page_name.lower().replace(" ", "_").replace("🏠", "").replace("🔍", "").replace(
                        "📝", "").replace("📊", "").replace("🎯", "").replace("💬", "").replace("ℹ️", "").strip()
                    st.session_state.page = cleaned_name
                    st.rerun()
            ...
            # if st.session_state.get('is_admin', False):
            #     st.success(
            #         f"Logged in as: {st.session_state.get('current_admin_email')}")
            #     if st.button("Logout", key="logout_button"):
            #         try:
            #             log_admin_action(st.session_state.get(
            #                 'current_admin_email'), "logout")
            #             st.session_state.is_admin = False
            ...
            #             "Email", key="admin_email_input")
            #         admin_password = st.text_input(
            #             "Password", type="password", key="admin_password_input")
